...
--local herpderp = data.raw["gui-style"]
function GUI.icon(name, iconName)
	if iconName == "crude-oil-barrel" then
		return GUI.parent().add{type = "checkbox", style = "item-icon-empty-barrel", state = false, name = name}
	end
	return GUI.parent().add{type = "checkbox", style = "item-icon-"..iconName, state = false, name = name}
	--return GUI.parent().add{type = "checkbox", state = false, name = name}
end

...
function Homeworld:spawn_reward( tier )
   -- This doesn't seem to work - always has instances?
   if #Portal._instances == 0 then
      PrintToAllPlayers("No portal exists to receive reward chest.")
      return
   end
   --Should find all homeworld portals on nauvis surface
   local portals = game.surfaces[1].find_entities_filtered{name="homeworld_portal"}
   PrintToAllPlayers(#portals)
   for index, name in pairs(portals) do
      PrintToAllPlayers(index)
   end
   --No portal entities, no reward
   if #portals == 0 then
      PrintToAllPlayers("No portal exists to receive reward chest.")
      return
   end
   -- Jadame: Redo this to select the first valid portal. If you get to the end and there are none, return
   -- As is, this also breaks if a portal gets destroyed.
   -- LuaSurface.find_entities_filtered looks like it would work!
